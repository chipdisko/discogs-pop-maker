...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!urlInfo.type || !urlInfo.id) {
      ...
      return NextResponse.json(
        { error: 'Invalid Discogs URL. Please provide a valid release, master, artist, or label URL.' },
        { status: 400 }
      );
    }
    ...
    if (!token) {
      ...
      return NextResponse.json(
        { error: 'Discogs API token not configured. Please set DISCOGS_API_TOKEN in .env.local' },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      switch (urlInfo.type) {
        case 'release':
          data = await new Promise((resolve, reject) => {
            db.getRelease(urlInfo.id, (err: any, release: any) => {
              if (err) reject(err);
        ...
        case 'master':
          data = await new Promise((resolve, reject) => {
            db.getMaster(urlInfo.id, (err: any, master: any) => {
              if (err) reject(err);
              else resolve(master);
            });
          });
        ...
        case 'artist':
          data = await new Promise((resolve, reject) => {
            db.getArtist(urlInfo.id, (err: any, artist: any) => {
              if (err) reject(err);
              else resolve(artist);
            });
          });
        ...
        case 'label':
          data = await new Promise((resolve, reject) => {
            db.getLabel(urlInfo.id, (err: any, label: any) => {
              if (err) reject(err);
              else resolve(label);
            });
          });
      ...
      console.log('API Route: Data keys:', Object.keys(data));
      ...
      if (discogsError.statusCode === 404) {
        return NextResponse.json(
      ...
      if (discogsError.statusCode === 401) {
        return NextResponse.json(
      ...
      if (discogsError.statusCode === 429) {
        return NextResponse.json(
      ...
      return NextResponse.json(
        { error: `Discogs API error: ${discogsError.message}` },
        { status: 500 }
      );
    }
