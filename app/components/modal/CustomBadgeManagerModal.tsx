...
import { Plus, Edit2, Trash2, Image, Type } from 'lucide-react';
...
export default function BadgeManagerModal({
  ...
  const handleSave = () => {
    try {
      ...
      const textError = BadgeStorageManager.validateBadgeText(formData.text || '');
      if (textError) {
        alert(textError);
        return;
      }
      ...
      alert(error instanceof Error ? error.message : 'バッジの保存に失敗しました');
    }
  };
  ...
  const renderBadgePreview = (badge: Badge | BadgeInput) => {
    ...
    if (badge.borderEnabled) {
      ...
      previewStyle.border = `${borderWidth}px solid ${badge.borderColor || '#ffffff'}`;
    }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
        <div className="p-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            最大{BADGE_LIMITS.MAX_COUNT}個まで作成できます ({badges.length}/{BADGE_LIMITS.MAX_COUNT})
          </p>
        </div>
            ...
            <div>
              ...
              {badges.length === 0 ? (
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                  ...
                  <p>カスタムバッジがありません</p>
                  <p className="text-sm">「新規作成」ボタンから最初のバッジを作成しましょう</p>
                </div>
              ) : (
                    ...
                    <div className="flex gap-4">
                      <label className="flex items-center">
                        <input
                          ...
                          onChange={(e) => setFormData(prev => ({ ...prev, shape: e.target.value as 'circle' | 'rectangle' }))}
                          className="mr-2"
                        />
                        ⭕ 円形
                      </label>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center">
                        <input
                          ...
                          onChange={(e) => setFormData(prev => ({ ...prev, borderEnabled: e.target.checked }))}
                          className="mr-2"
                        />
                        <span className="text-sm text-gray-700 dark:text-gray-300">枠線を表示する</span>
                      </div>
                        ...
                        <div className="grid grid-cols-2 gap-4 pl-6">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) => setFormData(prev => ({ ...prev, borderWidth: parseFloat(e.target.value) }))}
                              className="w-full"
                            />
                          </div>
                        </div>
